// ╔══════════════════════════════════════════════════╗
// ║  MEGAN-PRIME EVENT HANDLER - Clean v3.6.4       ║
// ║  Commands | Anti-Delete | ViewOnce | Status      ║
// ╚══════════════════════════════════════════════════╝

const { isJidGroup, isJidStatusBroadcast, downloadMediaMessage, areJidsSameUser } = require('megan-baileys');
...
const { setupGroupEvents } = require('../lib/events');
...
class EventHandler {
    ...
    async isOwner(jid) {
        ...
        if (this.ownerManager?.initialized) return this.ownerManager.isOwner(resolvedJid);
    ...
    buildQuotedMessage(msg) {
        try {
            const contextInfo = msg.message?.extendedTextMessage?.contextInfo ||
                ...
                msg.message?.audioMessage?.contextInfo || msg.message?.documentMessage?.contextInfo;
            if (!contextInfo) return null;
            const quotedMessage = contextInfo.quotedMessage;
            if (!quotedMessage) return null;
            return {
                key: { remoteJid: msg.key.remoteJid, id: contextInfo.stanzaId, participant: contextInfo.participant || msg.key.participant, fromMe: false },
                message: quotedMessage, participant: contextInfo.participant || msg.key.participant,
                pushName: contextInfo.pushName || msg.pushName, sender: contextInfo.participant || msg.key.remoteJid
            };
    ...
    getRandomQuote() { const q = this.getQuotes()[this.getTimeBlock()]; return q[Math.floor(Math.random() * q.length)]; }
    getEmojis() { return ['🔥','✅','❤️','💯','👍','🙌','👏','🎉','⭐','💫','✨','⚡','💪','👑','💎','🔮','🎯','🏆','💝','💖']; }
    getRandomEmoji() { return this.getEmojis()[Math.floor(Math.random() * this.getEmojis().length)]; }
    ...
    // ═══ STATUS HANDLING ═══
    async markAsRead(key) { try { await this.sock.readMessages([key]); } catch (error) {} }

    async autoReactToStatus(message, sender) {
        try {
            const emojisStr = await this.bot.db?.getSetting('status_react_emojis', '💛,❤️,💜,💙,👍,🔥');
            const emojis = emojisStr.split(',').map(e => e.trim());
            const emoji = emojis[Math.floor(Math.random() * emojis.length)];
            await this.sock.sendMessage('status@broadcast', { react: { key: message.key, text: emoji } });
        } catch (error) {}
    }
    ...
    async downloadAndSaveStatus(message, sender, text, mediaType) {
        try {
            ...
            if (!buffer) return;
            const filename = `status_${Date.now()}.${this.getExtension(mediaType)}`;
    ...
    async handleStatusDelete(deletedMsg, key, deleter) {
        try {
            ...
            if (antiDeleteStatus !== 'true') return;
            ...
            if (mediaType) {
                try {
                    ...
                    if (buffer) {
                        const filename = `deleted_status_${Date.now()}.${this.getExtension(mediaType)}`;
                        const { url, success } = await Uploader.uploadAuto(buffer, filename);
                        if (success && url) {
                            const caption = `🚨 *STATUS DELETED - ${mediaType.toUpperCase()}* 🚨\n\n👤 *From:* @${senderShort}\n🗑️ *Deleted by:* @${deleterShort}\n📎 *Type:* ${mediaType.toUpperCase()}\n${text ? `📝 *Caption:* ${text}\n` : ''}🔗 *Backup Link:* ${url}\n⏰ *Time:* ${currentTime}`;
                            const mentions = [sender, deleterResolved];
                            if (mediaType === 'image') await this.sock.sendMessage(this.bot.ownerJid, { image: buffer, caption, mentions });
                            else if (mediaType === 'video') await this.sock.sendMessage(this.bot.ownerJid, { video: buffer, caption, mentions });
                            else if (mediaType === 'audio') await this.sock.sendMessage(this.bot.ownerJid, { audio: buffer, mimetype: 'audio/mpeg', ptt: false, caption, mentions });
                            else await this.sock.sendMessage(this.bot.ownerJid, { document: buffer, fileName: filename, caption, mentions });
                            return;
                        }
                    }
                } catch (e) {}
    ...
    async handleViewOnce(message, from, sender) {
        try {
            ...
            let viewOnceContent = null, mediaType = null;

            if (msgContent.imageMessage?.viewOnce || msgContent.videoMessage?.viewOnce || msgContent.audioMessage?.viewOnce) {
                mediaType = Object.keys(msgContent).find(k => k.endsWith('Message') && ['image','video','audio'].some(t => k.includes(t)));
                ...
                mediaType = viewOnceContent ? Object.keys(viewOnceContent).find(k => k.endsWith('Message') && ['image','video','audio'].some(t => k.includes(t))) : null;
            ...
            if (this.bot.ownerJid) {
                ...
                if (type.includes('image')) await this.sock.sendMessage(this.bot.ownerJid, { image: buffer, caption, mentions: mention });
                else if (type.includes('video')) await this.sock.sendMessage(this.bot.ownerJid, { video: buffer, caption, mentions: mention });
                else if (type.includes('audio')) await this.sock.sendMessage(this.bot.ownerJid, { audio: buffer, ptt: true, mimetype: 'audio/mp4', caption, mentions: mention });
            }
        } catch (error) {}
    }

    // ═══ ANTI-LINK ═══
    isAnyLink(text) { return /https?:\/\/[^\s]+|www\.[^\s]+|bit\.ly\/[^\s]+/gi.test(text); }
        ...
        for (const [key, type] of Object.entries(mediaTypes)) {
            if (deletedMsg.message?.[key]) {
                try {
                    ...
                    const extensions = { image: 'jpg', video: 'mp4', audio: deletedMsg.message[key].mimetype?.includes('mpeg') ? 'mp3' : 'ogg', sticker: 'webp', document: deletedMsg.message[key].fileName?.split('.').pop() || 'bin' };
                    const filename = `deleted_${Date.now()}.${extensions[type]}`;
                    const { url, success } = await Uploader.uploadAuto(buffer, filename);
                    return { buffer, type, caption: deletedMsg.message[key].caption || '', mimetype: deletedMsg.message[key].mimetype, fileName: deletedMsg.message[key].fileName || filename, ptt: deletedMsg.message[key].ptt, url: success ? url : null };
                } catch (error) { return null; }
            }
        }
        return null;
    ...
    async handleAntiDelete(deletedMsg, key, deleter, sender) {
        try {
            ...
            if (antiDelete !== 'on' || !this.bot.ownerJid) return;
            ...
            try {
                const sc = await this.sock.getContact(resolvedSender).catch(() => null);
                if (sc?.name) senderName = sc.name;
                const dc = await this.sock.getContact(resolvedDeleter).catch(() => null);
                if (dc?.name) deleterName = dc.name;
            } catch (e) {}

            let chatInfo = 'Private Chat';
            if (isGroup) { try { const m = await this.sock.groupMetadata(key.remoteJid); chatInfo = `Group: ${m.subject}`; } catch (e) {} }
            ...
            if (deletedMsg.message?.conversation || deletedMsg.message?.extendedTextMessage?.text) {
                ...
                if (media?.buffer) {
                    ...
                    if (media.type === 'image') await this.sock.sendMessage(this.bot.ownerJid, { image: media.buffer, caption, mentions });
                    else if (media.type === 'video') await this.sock.sendMessage(this.bot.ownerJid, { video: media.buffer, caption, mentions });
                    else if (media.type === 'audio') await this.sock.sendMessage(this.bot.ownerJid, { audio: media.buffer, mimetype: 'audio/mpeg', ptt: media.ptt || false, caption, mentions });
                    else if (media.type === 'sticker') await this.sock.sendMessage(this.bot.ownerJid, { sticker: media.buffer, caption, mentions });
                    else await this.sock.sendMessage(this.bot.ownerJid, { document: media.buffer, fileName: media.fileName, mimetype: media.mimetype, caption, mentions });
                    if (media.url) await this.sock.sendMessage(this.bot.ownerJid, { text: `🔗 *Backup Link:*\n${media.url}`, mentions });
    ...
    async updateAutoBio() {
        try {
            ...
            const bio = `${config.BOT_NAME} Online • ${await this.getFormattedDate(Date.now())} • ${this.getRandomQuote()}`.substring(0, 139);
    ...
    async setPresence(jid) {
        try {
            ...
            let presence = await this.bot.db?.getSetting(isGroup ? 'presence_group' : 'presence_dm', 'typing');
            const map = { online: 'available', typing: 'composing', recording: 'recording' };
            const wp = map[presence?.toLowerCase()] || 'available';
            if (this.presenceTimers.has(jid)) clearTimeout(this.presenceTimers.get(jid));
            await this.sock.sendPresenceUpdate(wp, jid);
            this.presenceTimers.set(jid, setTimeout(() => this.presenceTimers.delete(jid), 15 * 60 * 1000));
        } catch (error) {}
    }
    ...
    async autoRead(msg) {
        try {
            ...
            if (setting !== 'on' || !msg.key || msg.key.fromMe || msg.key.remoteJid === 'status@broadcast') return;
    ...
    async handleAutoTyping(msg, from, isGroup) {
        try {
            ...
            if (setting !== 'on' || msg.key.fromMe) return;
    ...
    async handleCommand(msg, text, from, sender, isGroup) {
        ...
        if (!cmd && this.bot.aliases.has(commandName)) cmd = this.bot.commands.get(this.bot.aliases.get(commandName));
        ...
        if (cmd) {
            try {
                const context = {
                    ...
                    isOwner, uploader: Uploader, db: this.bot.db,
                    logger: this.logger, cache: this.cache, ownerManager: this.ownerManager,
                ...
                if (isOwner) await this.sock.sendMessage(from, { text: `❌ Error: ${error.message}` }, { quoted: msg });
            }
        }
    }
    ...
    async handleMessageUpdate(update, originalMsg = null) {
        try {
            ...
            if (msgUpdate?.message?.protocolMessage?.type === 0) {
                ...
                if (deletedMsg) {
                    if (key.remoteJid === 'status@broadcast') {
                        await this.handleStatusDelete(deletedMsg, key, key.participant || key.remoteJid);
                    } else {
                        await this.handleAntiDelete(deletedMsg, key, key.participant || key.remoteJid, deletedMsg.key?.participant || deletedMsg.key?.remoteJid);
                    }
            ...
            if (msgUpdate?.message?.protocolMessage?.type === 1 && originalMsg) {
                ...
                const editedText = this.extractText(msgUpdate.message);
                ...
                let chatInfo = 'Private Chat';
                if (isGroup) { try { const m = await this.sock.groupMetadata(key.remoteJid); chatInfo = `Group: ${m.subject}`; } catch (e) {} }
                const alertMsg = `✏️ *EDITED MESSAGE DETECTED*\n\n👤 *Sender:* @${resolvedSender.split('@')[0]}\n💬 *Chat:* ${chatInfo}\n⏰ *Time:* ${await this.getFormattedTime(Date.now())}\n📅 *Date:* ${await this.getFormattedDate(Date.now())}\n\n*Original:*\n${originalText}\n\n*Edited to:*\n${editedText}`;
                if (this.bot.ownerJid) await this.sock.sendMessage(this.bot.ownerJid, { text: alertMsg, mentions: [resolvedSender] });
            }

            if (msgUpdate?.reaction) this.logger.message(`Reaction: ${msgUpdate.reaction.text}`);
            if (msgUpdate?.status !== undefined) {
                const s = { 1: '📤 Sent', 2: '✅ Delivered', 3: '👁️ Read', 4: '🕒 Pending' };
                this.logger.message(`Status: ${s[msgUpdate.status] || msgUpdate.status}`);
            }
        } catch (error) {}
    }
        ...
        try {
            ...
            for (const key of keys) {
                ...
                if (cachedMsg && this.bot.db) {
                    const antiDelete = await this.bot.db.getSetting('antidelete', 'off');
                    if (antiDelete === 'on') {
                        await this.handleAntiDelete(cachedMsg, key, key.participant || key.remoteJid, cachedMsg.key?.participant || cachedMsg.key?.remoteJid);
                    }
    ...
    // ═══ GROUP UPDATE (delegated to events.js) ═══
    async handleGroupUpdate(update) {
        // Handled by megan/lib/events.js via setupGroupEvents()
        // Per-group settings, anti-promote, anti-demote, welcome/goodbye cards
    }
}
